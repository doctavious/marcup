use crate::parsers::span::Span;
use nom::error::{ContextError, ErrorKind, ParseError};
use std::{borrow::Cow, fmt};

/// Represents an encapsulated error that is encountered
#[derive(Clone, Debug, Default, Eq, PartialEq)]
pub struct MarcupParserError<'a> {
    ctx: Cow<'a, str>,
    input: Span<'a>,
    next: Option<Box<Self>>,
}

impl<'a> From<nom::Err<MarcupParserError<'a>>> for MarcupParserError<'a> {
    fn from(nom_err: nom::Err<MarcupParserError<'a>>) -> Self {
        match nom_err {
            nom::Err::Error(x) | nom::Err::Failure(x) => x,
            nom::Err::Incomplete(_) => Self::from_ctx(&Span::default(), "Incomplete"),
        }
    }
}

impl<'a> fmt::Display for MarcupParserError<'a> {
...
impl<'a> std::error::Error for MarcupParserError<'a> {}

impl<'a> MarcupParserError<'a> {
...
impl<'a> ParseError<Span<'a>> for MarcupParserError<'a> {
...
impl<'a> ContextError<Span<'a>> for MarcupParserError<'a> {

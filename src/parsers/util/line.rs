...
use crate::parsers::util::bytes::take_until_end_of_line_or_input;
...
mod tests {
    ...
    fn take_and_toss(n: usize) -> impl Fn(Span) -> IResult<()> {
        move |input: Span| nom::combinator::value((), nom::bytes::complete::take(n))(input)
    }

    #[test]
    fn beginning_of_line_should_fail_if_not_at_beginning_of_line() {
        let input = Span::from("1234");
        let (input, _) = take_and_toss(1)(input).expect("Failed to take a character");
    ...
    fn beginning_of_line_should_succeed_if_at_beginning_of_first_line() {
        let input = Span::from("1234");
        let (input, _) =
            beginning_of_line(input).expect("Unexpectedly think not at beginning of line");
    ...
    fn beginning_of_line_should_succeed_if_at_beginning_of_any_line() {
        ...
        let (input, _) = take_and_toss(4)(input).expect("Failed to take a character");
        let (input, _) =
            beginning_of_line(input).expect("Unexpectedly think not at beginning of line");
    ...
    fn any_line_should_return_empty_if_nothing_in_line() {
        let input = Span::from("\nabcd");
        let (input, content) = any_line(input).expect("Failed to parse any line");
        assert_eq!(input.as_unsafe_remaining_str(), "abcd");
